...
export default function CreateEventPage() {
  ...
  const availableSpeakers = speakerOptions.filter(
    (speaker) => !selectedSpeakers.includes(speaker),
  );
  const filteredSpeakerOptions = availableSpeakers.filter((speaker) =>
    speaker.toLowerCase().includes(speakerSearch.trim().toLowerCase()),
  );
  ...
  const addSpeaker = (speaker: string) => {
    if (!speaker || selectedSpeakers.includes(speaker)) {
      return;
    }

    setSelectedSpeakers((current) => [...current, speaker]);
    setSpeakerSearch("");
  };

  const removeSpeaker = (speaker: string) => {
    setSelectedSpeakers((current) =>
      current.filter((currentSpeaker) => currentSpeaker !== speaker),
    );
  };
      ...
      <div className="mx-auto max-w-5xl mt-6 space-y-6">
        <div>
          <SectionTitle
            ...
            description="        Fill in the information below to showcase your event to the campus
                    community."
          />{" "}
        </div>
        ...
        <form className="space-y-5">
          ...
          <div className="flex flex-wrap items-center justify-end gap-3 pt-2">
            <button
              type="submit"
              className="rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white shadow-sm shadow-blue-200 transition-colors hover:bg-blue-700"
            >
              Launch Event
            </button>
          </div>
